...
namespace Elroi.DailyMissions.UI
{
    public sealed class DailyMissionRowUI : MonoBehaviour
    {
        ...
        [SerializeField] Image missionIcon, progressFill, rewardIcon, secondaryRewardIcon;
        ...
        public void Bind(DailyMissionDefinition d,string localizedTitle,string localizedDescription,int p,bool claimed,string timer,Sprite cashRewardIcon,Sprite gemRewardIcon)
        {
            if(titleText)titleText.text=localizedTitle;if(descriptionText)descriptionText.text=localizedDescription;
            if(missionIcon&&d.missionIcon)missionIcon.sprite=d.missionIcon;
            bool showCurrencyPair=d.rewardCash>0&&d.rewardGems>0&&cashRewardIcon&&gemRewardIcon;
            if(rewardIcon)rewardIcon.sprite=showCurrencyPair?cashRewardIcon:d.rewardIcon;
            if(secondaryRewardIcon){secondaryRewardIcon.gameObject.SetActive(showCurrencyPair);if(showCurrencyPair)secondaryRewardIcon.sprite=gemRewardIcon;}
