...
namespace MoreMountains.InfiniteRunnerEngine
{
	public class Jumper : PlayableCharacter
	{
		...
		{
            if (_invLayerActive && !Invincible)
            {
                SetLayerRecursively(gameObject, _playerLayer);
				_invLayerActive = false;
            ...
            if (ShouldResetPosition)
            {
                if (_grounded && !_jumping)
                {
                    _rigidbodyInterface.Velocity =
                        new Vector3(
                            (_initialPosition.x - transform.position.x) * ResetPositionSpeed,
                            _rigidbodyInterface.Velocity.y,
